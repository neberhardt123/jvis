...
def create_diagram():
    ...
    config = [  'label: %component%', 
                ...
                'width: @width',
                'parentstyle: swimlane;fontStyle=1;childLayout=stackLayout;horizontal=1;startSize=26;fillColor=#60A917;horizontalStack=0;resizeParent=1;resizeParentMax=0;resizeLast=0;collapsible=1;fontColor=#ffffff;aspect=fixed;fontSize=14;whiteSpace=wrap;html=1',
                'parent: parent',
                'identity: identity',
                'height: @height',
                'left:%left%'
...
def create_topology():
    ...
    data = []
    attacker_parent = ["attacker_parent", "Attacker Network", "router", "sunet","180","200","","attacker_parent"]
    attacker = ["attacker","VDI Infrastructure","","atkr","80","76","","attacker"]
    ...
    for u in unique:
        subnet_parent = [u,"CHANGE ME{}{}".format("<br>",u),"","sunet","580","360","",u]
        data.append(subnet_parent)

    
    for box,n in zip(boxes,nets):
        subnet_child = [box.ip,"{}{}{}".format(box.ip,"<br>",box.hostname),"","comp","82","72","",box.ip]
        data.append(subnet_child)

    router_reference = "{}".format(','.join(unique))
    router = ["router","",router_reference,"rtr","122","56","","router"]
    data.append(router)
    styles = {
        ...
        "comp": "verticalLabelPosition=bottom;verticalAlign=top;fontColor=#000000;align=center;html=1;shape=mxgraph.gcp2.cloud_computer;whiteSpace=wrap;fillColor=#3B8DF1;",
        "sunet": "swimlane;html=1;fontStyle=1;startSize=45;fillColor=#3f7de0;collapsible=0;fontColor=#ffffff;fontSize=14;whiteSpace=wrap;html=1;"
    }
    config = [  'label: %name%',
                #'style: shape=%shape%;fillColor=%fill%;strokeColor=%stroke%;fontSize=%fontSize%;fontColor=%font%;aspect=fixed;%type%;image=%image%;align=center;whiteSpace=wrap;html=1',
                ...
                #'parentstyle: swimlane; fontStyle=1;html=1;childLayout=flowLayout;horizontal=1; fillColor=#3f7de0; fontColor=#ffffff;#whiteSpace=wrap; align=left;verticalAlign=top;recursiveResize=0;container=1;resizeParent=1;autoSize=0;',
                'stylename: cat',
...
def create_hostlist(c):
    response = None
    if c is not None:
        file_path = 'static/hostlist.txt'
        with open(file_path, 'w', newline='') as f:
            for item in c:
                f.write("{}\n".format(item))
        f.close()
